...
pub fn TokiPonaPage(cx: Scope) -> impl IntoView {
    ...
    view! { cx,
        ...
        <IjoAnteWindow pos=WindowPos::Val((310, 347)) size=(300, 255) hidden=ijo_ante_hidden     z_idx=z_idx/>
...
fn IjoAnteWindow(
    ...
    let active_tab = create_rw_signal(cx, "tan mi");
    ...
    let content = WindowContent::Tabs((
        ...
        vec![
            (
                "tan mi",
                view! { cx, <div style="padding: 5px" tabindex=0><p>
    ...
    view! {cx,
        <Window id="ijo-ante-win" title="ijo ante".to_string() content=content pos=pos size=size hidden=hidden z_idx=z_idx/>
    }
}
